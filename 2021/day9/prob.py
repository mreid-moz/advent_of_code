...
class Point:
  ...
  def __eq__(self, other):
    return other and self.r == other.r and self.c == other.c
  ...
  for p in points:
    ...
    if r > 0:
      if target < grid[r-1][c] and grid[r-1][c] < 9:
        new_points.append(Point(r-1, c))
    if c > 0:
      if target < grid[r][c-1] and grid[r][c-1] < 9:
        new_points.append(Point(r, c-1))
    if r < len(grid) - 1:
      if target < grid[r+1][c] and grid[r+1][c] < 9:
        new_points.append(Point(r+1, c))
    if c < len(grid[0]) - 1:
...
for row in range(len(my_input)):
  for col in range(len(my_input[0])):
    if is_low_point(my_input, row, col):
      ...
      basin_size = get_basin_size(my_input, [Point(row,col)], [])
